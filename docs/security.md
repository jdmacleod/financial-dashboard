...
There is no email-based password reset — HearthLedger runs locally with no mail server, so self-service reset would mean adding mail infrastructure to a single-household install. Recovery is operator-driven instead: whoever controls the host runs `scripts/reset_password.py <email>` (`AuthService.admin_reset_password`), which mints a one-time temporary password, forces a rotation on next login (`must_change_password`), clears any lockout, and invalidates existing sessions. This is the only path back in for a locked-out sole Primary, since it requires neither the current password nor an authenticated session. The reset writes an `auth.password_reset_admin` audit event; like every auth event, the row carries no password material. See [Account recovery](getting-started.md#account-recovery-forgotten-password) for the runbook.
