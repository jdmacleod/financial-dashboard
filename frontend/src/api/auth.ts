...
export const authApi = {
  ...
  reauth: (password: string) => api.post<{ reauth_token: string }>("/auth/reauth", { password }),

  changePassword: (current_password: string, new_password: string) =>
    api.post<void>("/auth/change-password", { current_password, new_password }),

  setup: (data: { household_name: string; member_name: string; email: string; password: string }) =>
