...
- **Retirement Milestones report.** A new report (`/reports/milestones`) shows each household member a forward-looking timeline of their age-based financial events: penalty-free retirement withdrawals at 59½, the earliest Social Security claim at 62, Medicare at 65, Social Security full retirement age (looked up by birth year), and the year required minimum distributions begin. Reached milestones are dimmed and the next upcoming one is badged, so a birthday turns into "RMDs begin 2041, Medicare 2033" instead of an inert number. Members with no date of birth get a prompt to add one.
...
- **FIRE projections account for required minimum distributions.** Once a member reaches RMD age, the projection draws the forced withdrawal from their pretax balance each year, and the year-by-year table gains a Required distribution column (shown only when an RMD actually applies in the window). Members with no pretax balance or no date of birth are unaffected.
