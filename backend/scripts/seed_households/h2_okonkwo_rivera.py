...
async def seed(session: AsyncSession, rng: random.Random) -> dict:
    ...
    session.add(
        make_insurance_policy(
            hid, "umbrella_liability", D("4000000"), D("780"), "annual",
    ...
    for who in (darius, carmen):
        session.add(
            make_insurance_policy(
                hid, "long_term_care", D("300000"), D("3200"), "annual",
                insured_member_id=who.id,
                metadata={"daily_benefit": 250, "inflation_rider": "3pct_compound"},
            )
        )
    for prem_year in (2024, 2025, 2026):
        add(tx(checking1.id, date(prem_year, 4, 10), -D("780.00"), "Chubb Umbrella Policy",
               cat["umbrella_premium"]))
        if prem_year < 2026:
            add(tx(checking1.id, date(prem_year, 10, 5), -D("6400.00"),
                   "Northwestern LTC (both)", cat["ltc_insurance_premium"]))

    # ── 529 superfunding (five-year-forward gift election) + private school ──────
    for kid_529 in (emma_529, noah_529):
        d, c = transfer(checking1.id, kid_529.id, date(2024, 1, 18), D("90000.00"),
                        "529 Superfund (5-yr election)", cat["529_superfund"])
        add(d, c)
    for month_start in all_months():
        y, m = month_start.year, month_start.month
        if m in (1, 2, 3, 4, 5, 8, 9, 10, 11, 12):  # 10 months, school year
            add(tx(checking1.id, clamp_day(y, m, 5), -D("1650.00"),
                   "Naperville Day School Tuition", cat["private_school_tuition"]))
