...
async def seed(session: AsyncSession, rng: random.Random) -> dict:
    ...
    ben_401k   = acc("retirement_401k",     "401(k) / Profit-Sharing Plan",  "Fidelity",                "8834", owner=ben.id)
    ...
    porsche_loan = acc("auto_loan",         "Porsche Cayenne",               "Porsche Financial",       "6659", owner=gabriela.id)
    ...
    session.add_all(build_snapshots(joint_brok.id, D("1175000.00"), joint_brok_contribs, 0.09, equity_dips))
    session.add_all(build_snapshots(ben_brok.id,  D("421000.00"),  ben_brok_contribs,  0.09, equity_dips))
    ...
    all_txns: list = []
    running: dict = {
        jpm_chk.id:   D("0"), jpm_sav.id:  D("0"), schwab_chk.id: D("0"),
        vanguard_mm.id: D("0"),
        amex_plat.id: D("0"), amex_gold.id: D("0"), chase_sap.id:  D("0"),
        jpm_mort.id:  D("0"), wf_mort.id:   D("0"), ld_mort.id:    D("0"),
        heloc.id:     D("0"), tesla_loan.id: D("0"), porsche_loan.id: D("0"),
    }
    ...
    str_income_range = {
        1: (5800, 7200), 2: (5800, 7200), 3: (5800, 7200), 4: (5800, 7200),
        5: (2400, 3200), 6: (0, 0), 7: (0, 0),
        8: (1800, 2600), 9: (1800, 2600),
        10: (4200, 5800), 11: (4200, 5800),
        12: (3200, 4400),
    }
    ...
    for month_start in all_months():
        ...
        add(tx(jpm_chk.id, clamp_day(y, m, 1), D("18500.00"),
               "Whitfield & Associates LLP", cat["salary"]))
        add(tx(jpm_chk.id, clamp_day(y, m, 15), D("24000.00"),
               "Torres Dev Consulting LLC", cat["consulting_fees"]))

        # Silver Lake rental income (tagged to prop_silver)
        unit_a_amt = D("3200.00")
        unit_b_amt = D("0.00") if (y == 2024 and m == 11) else D("2950.00")
        add(tx(jpm_chk.id, clamp_day(y, m, 1), unit_a_amt,
               "Tenant ACH — Unit A", cat["residential_rental"], prop_id=prop_silver.id))
        if unit_b_amt > 0:
            add(tx(jpm_chk.id, clamp_day(y, m, 1), unit_b_amt,
                   "Tenant ACH — Unit B", cat["residential_rental"], prop_id=prop_silver.id))

        # Palm Springs STR income (tagged to prop_ps, deposited 15th)
        lo, hi = str_income_range[m]
        if hi > 0:
            str_amt = jitter(D(str((lo + hi) // 2)), rng, 0.15)
            add(tx(jpm_chk.id, clamp_day(y, m, 15), str_amt,
                   "Airbnb Payout", cat["short_term_rental"], prop_id=prop_ps.id))

        # W&A LLP Annual Distribution (October to Schwab Checking)
        if m == 10:
            add(tx(schwab_chk.id, clamp_day(y, m, 20), D("120000.00"),
                   "W&A LLP Partner Distribution", cat["profit_distribution"]))

        # CA Tax refund (April)
        if m == 4:
            add(tx(jpm_chk.id, clamp_day(y, m, 25), D("8400.00"),
                   "CA Franchise Tax Board", cat["tax_refund"]))

        # ── Fixed outflows from JPM Checking ──────────────────────────────────
        d, c = transfer(jpm_chk.id, jpm_mort.id, clamp_day(y, m, 5),
                        D("15050.00"), "JPM Chase Private Mortgage", cat["mortgage_payment"])
        add(d, c)
        d, c = transfer(jpm_chk.id, wf_mort.id, clamp_day(y, m, 5),
                        D("5170.00"), "Wells Fargo — Silver Lake", cat["mortgage_payment"],
                        prop_id=prop_silver.id)
        add(d, c)
        d, c = transfer(jpm_chk.id, ld_mort.id, clamp_day(y, m, 5),
                        D("4876.00"), "LoanDepot — Palm Springs", cat["mortgage_payment"],
                        prop_id=prop_ps.id)
        add(d, c)

        # HELOC: interest charge (from Chase) + payment from checking
        add(tx(heloc.id, clamp_day(y, m, 10), -D("920.00"),
               "Chase HELOC Interest", cat["heloc_payment"]))
        d, c = transfer(jpm_chk.id, heloc.id, clamp_day(y, m, 15),
                        D("920.00"), "Chase HELOC Payment", cat["heloc_payment"])
        add(d, c)

        d, c = transfer(jpm_chk.id, porsche_loan.id, clamp_day(y, m, 22),
                        D("752.00"), "Porsche Financial", cat["loan_payment"])
        add(d, c)
        d, c = transfer(jpm_chk.id, joint_brok.id, clamp_day(y, m, 15),
                        D("5000.00"), "Schwab Auto-Invest", cat["brokerage_contribution"])
        add(d, c)

        # Ethan 529 contribution (through July 2025)
        if date(y, m, 1) <= date(2025, 7, 1):
            d, c = transfer(jpm_chk.id, ethan_529.id, clamp_day(y, m, 10),
                            D("2333.00"), "ScholarShare 529 — Ethan", cat["brokerage_contribution"])
            add(d, c)

        # Sophia's Roth IRA (from Ben, as gift)
        d, c = transfer(jpm_chk.id, sophia_roth.id, clamp_day(y, m, 10),
                        D("500.00"), "Ben → Sophia Roth IRA", cat["ira_contribution"])
        add(d, c)
        ...
        add(tx(jpm_chk.id, clamp_day(y, m, 15), -D("645.00"), "AIG Private Client Auto", cat["auto_insurance"]))
        add(tx(jpm_chk.id, clamp_day(y, m, 15), -D("520.00"), "Chubb Masterpiece Home", cat["home_insurance"]))
        add(tx(jpm_chk.id, clamp_day(y, m, 15), -D("1800.00"), "Housekeeper", cat["cleaning_services"]))
        add(tx(jpm_chk.id, clamp_day(y, m, 15), -D("680.00"), "Gardener", cat["lawn_garden"]))
        add(tx(jpm_chk.id, clamp_day(y, m, 15), -D("1240.00"), "Pacific Life Insurance", cat["life_insurance"]))

        # Advisory fees — quarterly ($5,400 in Jan/Apr/Jul/Oct)
        if m in (1, 4, 7, 10):
            add(tx(jpm_chk.id, clamp_day(y, m, 20), -D("5400.00"),
                   "Schwab Private Client Advisory", cat["advisory_fees"]))

        # Silver Lake Duplex expenses (tagged to prop_silver)
        sl_maint = jitter(D("1200.00"), rng, 0.40)
        add(tx(jpm_chk.id, clamp_day(y, m, rng.randint(10, 20)), -sl_maint,
               "Contractor — Silver Lake", cat["rental_maintenance"], prop_id=prop_silver.id))
        add(tx(jpm_chk.id, clamp_day(y, m, 10), -D("285.00"),
               "Farmers Landlord Policy", cat["rental_insurance"], prop_id=prop_silver.id))
        if m in (4, 10):
            add(tx(jpm_chk.id, clamp_day(y, m, 20), -D("10500.00"),
                   "LA County Assessor — Property Tax", cat["rental_property_tax"],
                   prop_id=prop_silver.id))
        ...
        if str_income_range[m][1] > 0:
            ...
            add(tx(jpm_chk.id, clamp_day(y, m, rng.randint(10, 20)), -ps_maint,
                   "PS Contractor", cat["rental_maintenance"], prop_id=prop_ps.id))
            lo2, hi2 = str_income_range[m]
            str_gross = jitter(D(str((lo2 + hi2) // 2)), rng, 0.15)
            mgmt_fee = (str_gross * D("0.15")).quantize(D("0.01"))
            add(tx(jpm_chk.id, clamp_day(y, m, 20), -mgmt_fee,
                   "Stay Duvet Property Management", cat["property_management"],
                   prop_id=prop_ps.id))
        add(tx(jpm_chk.id, clamp_day(y, m, 10), -D("320.00"),
               "Proper Insurance STR Policy", cat["rental_insurance"], prop_id=prop_ps.id))
        if m in (3, 11):
            add(tx(jpm_chk.id, clamp_day(y, m, 20), -D("5800.00"),
                   "Riverside County — Property Tax", cat["rental_property_tax"],
                   prop_id=prop_ps.id))

        # ── Schwab Checking fixed outflows ─────────────────────────────────────
        # Tesla loan (with $500 extra)
        d, c = transfer(schwab_chk.id, tesla_loan.id, clamp_day(y, m, 22),
                        D("1788.00"), "Tesla Financial", cat["loan_payment"])
        add(d, c)
        # Ben's individual brokerage
        d, c = transfer(schwab_chk.id, ben_brok.id, clamp_day(y, m, 15),
                        D("2000.00"), "Fidelity — Individual Brokerage", cat["brokerage_contribution"])
        add(d, c)

        # Roth IRA contributions Jan-Oct (from JPM Checking)
        if m in range(1, 11):
            d, c = transfer(jpm_chk.id, ben_roth.id, clamp_day(y, m, 5),
                            D("583.00"), "Fidelity — Backdoor Roth IRA", cat["ira_contribution"])
            add(d, c)
            d, c = transfer(jpm_chk.id, gab_roth.id, clamp_day(y, m, 5),
                            D("583.00"), "Vanguard — Backdoor Roth IRA", cat["ira_contribution"])
            add(d, c)
        ...
        def plat_var(slug, merchants, mn, mx, min_n, max_n):
            plat_txns.extend(gen_variable(amex_plat.id, y, m, cat[slug],
                                           merchants, D(str(mn)), D(str(mx)), min_n, max_n, rng))

        plat_var("restaurants", ["Nobu", "Jon & Vinny's", "Spago", "Republique", "Osteria Mozza"],
                 1400, 2800, 5, 10)
        plat_txns.append(tx(amex_plat.id, clamp_day(y, m, 5), -D("320.00"), "AT&T (4 lines)", cat["cell_phone"]))
        for sub, amt in [("Netflix 4K", D("22.99")), ("HBO Max", D("15.99")),
                          ("Apple TV+", D("9.99")), ("Spotify Family", D("16.99")),
                          ("Peloton App", D("24.99"))]:
            plat_txns.append(tx(amex_plat.id, clamp_day(y, m, 7), -amt, sub, cat["streaming"]))
        plat_txns.append(tx(amex_plat.id, clamp_day(y, m, 5), -D("390.00"), "Equinox", cat["fitness"]))
        plat_txns.append(tx(amex_plat.id, clamp_day(y, m, 8), -D("880.00"), "Therapy Sessions", cat["therapy"]))
        plat_var("clothing", ["Nordstrom", "Saks Fifth Avenue", "Maxfield LA", "Mr Porter"],
                 800, 2200, 1, 4)
        plat_var("ev_charging", ["Tesla Supercharger", "ChargePoint"], 65, 120, 2, 5)
        for sub in [("Amex Centurion Lounge", D("50.00")), ("WSJ", D("30.00")),
                     ("FT Digital", D("40.00")), ("NYT", D("25.00")), ("LinkedIn Premium", D("40.00"))]:
            plat_txns.append(tx(amex_plat.id, clamp_day(y, m, 8), -sub[1], sub[0], cat["subscriptions"]))
        if rng.random() < 0.50:
            plat_var("home_goods", ["RH Brentwood", "Williams-Sonoma", "Crate & Barrel"], 400, 2000, 0, 2)

        # ── Amex Gold spending (Gabriela's card) — paid from JPM Checking ─────
        gold_txns: list = []

        def gold_var(slug, merchants, mn, mx, min_n, max_n, **kw):
            gold_txns.extend(gen_variable(amex_gold.id, y, m, cat[slug],
                                           merchants, D(str(mn)), D(str(mx)), min_n, max_n, rng, **kw))

        gold_var("groceries", ["Erewhon", "Bristol Farms", "Gelson's", "Costco"],
                 1800, 2400, 6, 10, avoid_sunday=True)
        gold_var("coffee", ["Starbucks Reserve", "Blue Bottle", "Intelligentsia"],
                 120, 180, 5, 10)
        gold_var("food_delivery", ["Uber Eats", "Postmates", "DoorDash"],
                 200, 350, 3, 6)
        gold_var("personal_care", ["Salon Cristophe", "Burke Williams Spa", "Drybar"],
                 600, 1100, 3, 6)
        gold_var("pet_care", ["VCA Animal Hospitals", "Pet Groomer", "Petco"],
                 180, 450, 2, 4)

        # Chase Sapphire — gas + occasional shared spending
        chase_txns: list = []
        chase_txns.append(tx(chase_sap.id, clamp_day(y, m, 15), -D("95.00"),
                              "Chevron", cat["gas_fuel"]))
        ...
        if m == 7:
            plat_var("travel", ["Lufthansa First", "Four Seasons Paris", "Hertz Europe"], 18000, 28000, 3, 6)
        if m in (9, 12, 3):
            # UCSB tuition (Ethan)
            add(tx(jpm_chk.id, clamp_day(y, m, 15), -D("4800.00"),
                   "UCSB Fees", cat["tuition"]))
        if m == 12:
            plat_var("gifts_given", ["Neiman Marcus", "Amazon", "Amorim Beverly Hills"], 5000, 8000, 3, 7)
        if m == 4:
            add(tx(jpm_chk.id, clamp_day(y, m, 20), -D("12500.00"),
                   "Holthouse Carlin & Van Trigt LLP", cat["tax_prep"]))
        if m == 2:
            add(tx(jpm_chk.id, clamp_day(y, m, 15), -D("4200.00"),
                   "Estate Attorney Annual Review", cat["professional_services"]))
        if m == 8:
            hm_amt = jitter(D("4500.00"), rng, 0.25)
            add(tx(jpm_chk.id, clamp_day(y, m, 15), -hm_amt,
                   "Pool Service & HVAC", cat["home_maintenance"]))

        # ── CC payments ───────────────────────────────────────────────────────
        plat_total = sum(abs(t.amount) for t in plat_txns)
        if plat_total > 0:
            d, c = transfer(schwab_chk.id, amex_plat.id, clamp_day(y, m, 28),
                            plat_total, "Amex Platinum Statement Payment", cat["cc_payment"])
            add(d, c)

        gold_total = sum(abs(t.amount) for t in gold_txns)
        if gold_total > 0:
            d, c = transfer(jpm_chk.id, amex_gold.id, clamp_day(y, m, 27),
                            gold_total, "Amex Gold Statement Payment", cat["cc_payment"])
            add(d, c)

        chase_total = sum(abs(t.amount) for t in chase_txns)
        if chase_total > 0:
            d, c = transfer(jpm_chk.id, chase_sap.id, clamp_day(y, m, 27),
                            chase_total, "Chase Sapphire Statement Payment", cat["cc_payment"])
            add(d, c)

        # Periodic transfer JPM Checking → JPM Savings
        if rng.random() < 0.65:
            sav_amt = D(str(rng.choice([5000, 10000, 15000])))
            d, c = transfer(jpm_chk.id, jpm_sav.id, clamp_day(y, m, 26),
                            sav_amt, "Transfer to Private Client Savings", cat["savings_transfer"])
            add(d, c)
    ...
    # ── FIRE scenarios ─────────────────────────────────────────────────────────
    fire_a_streams = [
        {"id": str(uuid.uuid4()), "label": "Ben — Law Firm (until 58)", "type": "salary",
         "amount_annual": 650000.00, "start_year": 2024, "end_year": 2030, "growth_rate_annual": 0.04},
        {"id": str(uuid.uuid4()), "label": "Gabriela — Consulting (full)", "type": "consulting",
         "amount_annual": 385000.00, "start_year": 2024, "end_year": 2030, "growth_rate_annual": 0.03},
        {"id": str(uuid.uuid4()), "label": "Gabriela — Consulting (reduced, post-56)", "type": "consulting",
         "amount_annual": 120000.00, "start_year": 2030, "end_year": None, "growth_rate_annual": 0.02},
        {"id": str(uuid.uuid4()), "label": "Silver Lake Duplex — Net Rental", "type": "rental",
         "amount_annual": 58000.00, "start_year": 2024, "end_year": None, "growth_rate_annual": 0.02},
        {"id": str(uuid.uuid4()), "label": "Palm Springs STR — Net", "type": "rental",
         "amount_annual": 32000.00, "start_year": 2024, "end_year": None, "growth_rate_annual": 0.01},
        {"id": str(uuid.uuid4()), "label": "Benjamin Social Security (age 70)", "type": "social_security",
         "amount_annual": 65000.00, "start_year": 2042, "end_year": None, "growth_rate_annual": 0.025},
        {"id": str(uuid.uuid4()), "label": "Gabriela Social Security (age 70)", "type": "social_security",
         "amount_annual": 52000.00, "start_year": 2044, "end_year": None, "growth_rate_annual": 0.025},
    ]
    session.add(make_fire_scenario(
        hid, ben.id, "Coast at 58 — Semi-Retirement",
        D("420000.00"), D("0.0650"), D("0.0300"), 58, fire_a_streams,
    ))
    ...
    session.add(make_fire_scenario(
        hid, ben.id, "True FIRE — Stress Test",
        D("380000.00"), D("0.0550"), D("0.0350"), 54, fire_b_streams,
    ))

    # ── Debt record ───────────────────────────────────────────────────────────
    session.add(make_debt(
        tesla_loan.id,
        D("58000.00"), D("38200.00"),
        D("0.0790"), D("1288.00"), 72, date(2023, 2, 1),
    ))
