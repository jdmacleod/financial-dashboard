...
async def seed(session: AsyncSession, rng: random.Random) -> dict:
    ...
    rav4_loan    = acc("auto_loan",             "RAV4 Auto Loan",      "Toyota Financial Services",      "2205", owner=priya.id)
    ...
    session.add_all(build_snapshots(brokerage.id,  D("51200.00"),  brokerage_contribs, 0.09, dips_market))
    ...
    running: dict[uuid.UUID, D] = {
        checking.id: D("0"), savings.id: D("0"),
        cc_sapphire.id: D("0"), rav4_loan.id: D("0"), mortgage.id: D("0"),
    }
    ...
    for month_start in all_months():
        ...
        if m == 4:
            add(tx(checking.id, clamp_day(y, m, 15), D("2100.00"), "IRS TREAS 310", cat["tax_refund"]))

        # ── Fixed checking outflows ────────────────────────────────────────────
        # Mortgage (transfer to mortgage account)
        d, c = transfer(checking.id, mortgage.id, clamp_day(y, m, 5),
                        D("1828.00"), "University Federal CU", cat["mortgage_payment"])
        add(d, c)

        # Utilities (direct from checking)
        add(tx(checking.id, clamp_day(y, m, 10), -D("62.00"), "City of Round Rock Utilities", cat["water_sewer"]))
        ...
        d, c = transfer(checking.id, brokerage.id, clamp_day(y, m, 15),
                        D("1000.00"), "Fidelity Auto-Invest", cat["brokerage_contribution"])
        add(d, c)

        # Auto loan payment
        d, c = transfer(checking.id, rav4_loan.id, clamp_day(y, m, 22),
                        D("312.00"), "Toyota Financial Services", cat["loan_payment"])
        add(d, c)

        # IRA contributions Jan–May
        if m in (1, 2, 3, 4, 5):
            d, c = transfer(checking.id, wei_roth.id, clamp_day(y, m, 11),
                            D("1400.00"), "Fidelity — Roth IRA", cat["ira_contribution"])
            add(d, c)
            d, c = transfer(checking.id, priya_roth.id, clamp_day(y, m, 10),
                            D("1400.00"), "Vanguard — Roth IRA", cat["ira_contribution"])
            add(d, c)

        # ── CC variable spending ───────────────────────────────────────────────
        cc_this_month: list = []
        def cc_var(cat_slug, merchants, min_t, max_t, min_n, max_n, **kw):
            txns = gen_variable(cc_sapphire.id, y, m, cat[cat_slug],
                                merchants, D(str(min_t)), D(str(max_t)), min_n, max_n, rng, **kw)
            cc_this_month.extend(txns)

        cc_var("groceries", ["H-E-B", "Costco", "Trader Joe's", "H-E-B Gas Station"],
               680, 820, 6, 8, avoid_sunday=True)
        cc_var("restaurants", ["Torchy's Tacos", "Chuy's", "Pappasito's", "Uchi Austin", "Local Austin Bistro"],
               280, 420, 4, 7)
        cc_var("coffee", ["Starbucks", "Epoch Coffee", "Houndstooth Coffee"],
               55, 95, 4, 8)
        cc_var("food_delivery", ["DoorDash", "Uber Eats"],
               45, 90, 2, 4)
        cc_var("gas_fuel", ["H-E-B Gas", "Shell", "ExxonMobil"],
               140, 180, 3, 5)

        # Fixed CC charges
        cc_this_month.append(tx(cc_sapphire.id, clamp_day(y, m, 3), -D("75.00"), "AT&T Fiber", cat["internet"]))
        cc_this_month.append(tx(cc_sapphire.id, clamp_day(y, m, 5), -D("110.00"), "T-Mobile", cat["cell_phone"]))
        for streaming_payee, streaming_amt in [("Netflix", D("17.00")), ("Spotify", D("10.99")), ("Disney+", D("14.00"))]:
            cc_this_month.append(tx(cc_sapphire.id, clamp_day(y, m, 7), -streaming_amt, streaming_payee, cat["streaming"]))
        cc_this_month.append(tx(cc_sapphire.id, clamp_day(y, m, 15), -D("186.00"), "USAA Auto Insurance", cat["auto_insurance"]))
        cc_this_month.append(tx(cc_sapphire.id, clamp_day(y, m, 18), -D("89.00"), "Life Time Fitness", cat["fitness"]))

        # Home insurance: Jan only
        if m == 1:
            cc_this_month.append(tx(cc_sapphire.id, date(y, 1, 8), -D("165.00"), "USAA Home Insurance", cat["home_insurance"]))

        cc_var("clothing", ["Target", "Amazon", "Old Navy"], 85, 200, 1, 3)
        cc_var("personal_care", ["CVS Beauty", "Ulta", "Great Clips"], 60, 120, 2, 3)
        for sub_payee in [("Amazon Prime", D("14.99")), ("NYT", D("17.00")), ("Xbox Game Pass", D("15.00"))]:
            cc_this_month.append(tx(cc_sapphire.id, clamp_day(y, m, 8), -sub_payee[1], sub_payee[0], cat["subscriptions"]))
        ...
        if m == 4:
            cc_this_month.append(tx(cc_sapphire.id, clamp_day(y, m, 20), -D("180.00"), "TurboTax", cat["tax_prep"]))
        if m == 1:
            cc_this_month.append(tx(cc_sapphire.id, date(y, 1, 20), -D("250.00"), "ABC Pest Control & HVAC", cat["home_maintenance"]))
        if m == 6:
        ...
        if cc_month_total > 0:
            ...
            d, c = transfer(checking.id, cc_sapphire.id, pay_day,
                            cc_month_total, "Chase Sapphire Statement Payment", cat["cc_payment"])
            add(d, c)

    # ── Savings transfer (checking → savings each month) ───────────────────────
    for month_start in all_months():
        y, m = month_start.year, month_start.month
        d, c = transfer(checking.id, savings.id, clamp_day(y, m, 25),
                        D("1500.00"), "Transfer to High-Yield Savings", cat["savings_transfer"])
        add(d, c)
    ...
    fire = make_fire_scenario(
        ...
        55,
        [
            {"id": str(uuid.uuid4()), "label": "Wei — Dell salary", "type": "salary",
             "amount_annual": 115000.00, "start_year": 2024, "end_year": 2040,
             "growth_rate_annual": 0.03, "is_pre_retirement": True},
            {"id": str(uuid.uuid4()), "label": "Priya — RN salary", "type": "salary",
             "amount_annual": 98000.00, "start_year": 2024, "end_year": 2039,
             "growth_rate_annual": 0.02, "is_pre_retirement": True},
            {"id": str(uuid.uuid4()), "label": "Wei Social Security (age 67)", "type": "social_security",
             "amount_annual": 38000.00, "start_year": 2051, "end_year": None,
             "growth_rate_annual": 0.025, "is_pre_retirement": False},
            {"id": str(uuid.uuid4()), "label": "Priya Social Security (age 67)", "type": "social_security",
             "amount_annual": 32000.00, "start_year": 2053, "end_year": None,
             "growth_rate_annual": 0.025, "is_pre_retirement": False},
        ],
    )
    session.add(fire)
    ...
    session.add(make_debt(
        rav4_loan.id,
        D("23500.00"), D("12400.00"),
        D("0.0589"), D("312.00"), 60, date(2022, 3, 1),
    ))
    ...
    net_worth += D("210400.00") + D("95200.00") + D("48100.00") + D("32200.00") + D("72800.00") + D("13100.00")
