...
async def seed(session: AsyncSession, rng: random.Random) -> dict:
    ...
    llc_checking   = acc("checking",            "Consulting LLC Checking",    "Regions Bank",        "2170", owner=maggie.id)
    ...
    bob_brok       = acc("investment_brokerage","Bob Individual Brokerage",   "Fidelity",            "8736", owner=bob.id)
    ...
    jb_contribs = {last_day_of(y, m): D("2000.00")
                   for ms in all_months() if ms < date(2026, 6, 1)
                   for y, m in [(ms.year, ms.month)]}
    session.add_all(build_snapshots(joint_brok.id, D("2780000.00"), jb_contribs, 0.065, brokerage_dips))

    # Bob Individual Brokerage: $1,000/mo; 6.5%; same dips
    bb_contribs = {last_day_of(y, m): D("1000.00")
                   for ms in all_months() if ms < date(2026, 6, 1)
                   for y, m in [(ms.year, ms.month)]}
    session.add_all(build_snapshots(bob_brok.id, D("612000.00"), bb_contribs, 0.065, brokerage_dips))
    ...
    for month_start in all_months():
        ...
        add(tx(checking.id, clamp_day(y, m, 1), -med_b_amt,
               "Medicare — Part B Premium", cat["medicare_part_b"]))
        add(tx(checking.id, clamp_day(y, m, 1), -med_d_amt,
               "Medicare — Part D Premium", cat["medicare_part_d"]))
        add(tx(checking.id, clamp_day(y, m, 3), -medigap_amt,
               "UnitedHealthcare Medigap Plan G", cat["medigap_supplement"]))
        ...
        if y < 2025:
            ...
            aca_amt = D("1310.00")
        add(tx(checking.id, clamp_day(y, m, 5), -aca_amt,
               "BCBS Florida ACA Marketplace", cat["aca_premium"]))

        # ── Highlands NC mortgage ─────────────────────────────────────────────
        d, c = transfer(checking.id, highlands_mort.id, clamp_day(y, m, 1),
                        D("1632.00"), "Bank of America Mortgage — Highlands NC",
                        cat["mortgage_payment"])
        add(d, c)

        # ── Sarasota HOA + condo fees ─────────────────────────────────────────
        add(tx(checking.id, clamp_day(y, m, 10), -D("895.00"),
               "Osprey Point HOA", cat["hoa_fees"]))

        # ── Utilities (Sarasota) ──────────────────────────────────────────────
        add(tx(checking.id, clamp_day(y, m, 15), -D("145.00"),
               "Florida Power & Light", cat["electric"]))
        add(tx(checking.id, clamp_day(y, m, 15), -D("68.00"),
               "Comcast Xfinity — Sarasota", cat["internet"]))
        add(tx(checking.id, clamp_day(y, m, 15), -D("75.00"),
               "T-Mobile — Bob & Maggie", cat["cell_phone"]))

        # ── Life and umbrella insurance (annual policy, billed monthly) ───────
        add(tx(checking.id, clamp_day(y, m, 20), -D("510.00"),
               "USAA Life & Umbrella Insurance", cat["life_insurance"]))

        # ── Household cleaning (Sarasota condo) ──────────────────────────────
        add(tx(checking.id, clamp_day(y, m, 8), -D("225.00"),
               "Sparkling Sarasota Cleaning", cat["cleaning_services"]))

        # ── Investment advisory fee (quarterly, end of Q) ─────────────────────
        if m in (3, 6, 9, 12):
            advisory_q = D("6500.00") if m in (3, 9) else D("7000.00")
            add(tx(checking.id, last_day_of(y, m), -advisory_q,
                   "Truist Wealth Management Advisory Fee", cat["advisory_fees"]))

        # ── Roth IRA backdoor contributions Jan-Oct ───────────────────────────
        if m <= 10:
            d, c = transfer(checking.id, bob_roth.id, clamp_day(y, m, 12),
                            D("583.00"), "Fidelity — Bob Roth IRA (Backdoor)",
                            cat["ira_contribution"])
            add(d, c)
            d, c = transfer(checking.id, maggie_roth.id, clamp_day(y, m, 13),
                            D("583.00"), "Vanguard — Maggie Roth IRA (Backdoor)",
                            cat["ira_contribution"])
            add(d, c)

        # ── Joint brokerage auto-invest ───────────────────────────────────────
        d, c = transfer(checking.id, joint_brok.id, clamp_day(y, m, 15),
                        D("2000.00"), "Schwab — Joint Brokerage Auto-Invest",
                        cat["brokerage_contribution"])
        add(d, c)
        d, c = transfer(checking.id, bob_brok.id, clamp_day(y, m, 16),
                        D("1000.00"), "Fidelity — Bob Brokerage Auto-Invest",
                        cat["brokerage_contribution"])
        add(d, c)

        # ── Income: Bob Social Security (3rd Wednesday, net of Medicare) ──────
        ss_date = third_wednesday(y, m)
        add(tx(checking.id, ss_date, D("4886.00"),
               "US Treasury Social Security", cat["social_security_income"]))

        # ── Income: Bob Meridian Packaging Pension (1st of month) ────────────
        add(tx(checking.id, clamp_day(y, m, 1), D("4000.00"),
               "Meridian Packaging Pension", cat["pension_income"]))

        # ── Income: Bob IRA RMD (quarterly, 0 in 2024) ───────────────────────
        if y == 2025 and m in (3, 6, 9):
            rmd_amt = D("34896.00")
            add(tx(checking.id, last_day_of(y, m), rmd_amt,
                   "Schwab IRA Distribution — RMD", cat["rmd_distribution"]))
        elif y == 2025 and m == 12:
            add(tx(checking.id, clamp_day(y, m, 15), D("34897.00"),
                   "Schwab IRA Distribution — RMD", cat["rmd_distribution"]))
        elif y == 2026 and m == 3:
            add(tx(checking.id, last_day_of(y, m), D("36530.00"),
                   "Schwab IRA Distribution — RMD", cat["rmd_distribution"]))
        ...
        if m in _joint_div_by_q:
            ...
            if q_last <= DATE_END:
                add(tx(checking.id, q_last, _joint_div_by_q[m],
                       "Schwab Brokerage Dividend", cat["dividends"]))
                add(tx(checking.id, q_last, _bob_div_by_q[m],
                       "Fidelity Brokerage Dividend", cat["dividends"]))
        ...
        for i, rev in enumerate(rev_parts):
            rev_day = clamp_day(y, m, rng.choice([5, 8, 12, 18, 22, 26]))
            add(tx(llc_checking.id, rev_day, rev,
                   f"Client Invoice — HR Consulting", cat["consulting_fees"]))

        # LLC expenses
        add(tx(llc_checking.id, clamp_day(y, m, 5), -D("299.00"),
               "Microsoft 365 Business", cat["marketing_software"]))
        add(tx(llc_checking.id, clamp_day(y, m, 7), -D("150.00"),
               "SHRM Membership", cat["professional_dev"]))
        if rng.random() < 0.4:
            add(tx(llc_checking.id, clamp_day(y, m, rng.randint(10, 20)),
                   -D(str(rng.randint(8, 22) * 50)),
                   "1099 Contractor — Research", cat["professional_services"]))
        if rng.random() < 0.25:
            add(tx(llc_checking.id, clamp_day(y, m, rng.randint(5, 20)),
                   -D(str(rng.randint(2, 8) * 50)),
                   "Business Travel — Client Site", cat["business_travel"]))

        # Monthly draw from LLC to joint checking (15th)
        draw_amt = D("3200.00")
        d, c = transfer(llc_checking.id, checking.id, clamp_day(y, m, 15),
                        draw_amt, "Langford HR Consulting LLC — Owner Draw",
                        cat["consulting_fees"])
        add(d, c)
        ...
        def cv(slug, merchants, min_t, max_t, min_n, max_n, **kw):
            cen_txns.extend(gen_variable(
                centurion.id, y, m, cat[slug], merchants,
                D(str(min_t)), D(str(max_t)), min_n, max_n, rng, **kw))

        cv("restaurants",
           ["Selby Gardens Cafe", "Klassic Chophouse", "Michael's On East",
            "Ocean Prime Sarasota", "St. Armands Circle Restaurant"],
           1100, 1800, 3, 5)
        cv("home_goods",
        ...
        if m == 2:
            cv("travel",
               ["Delta Air Lines", "Marriott Bonvoy", "Hertz Premium"],
               6000, 11000, 2, 4)
        elif m == 6:
            cv("travel",
            ...
            cv("travel",
               ["Sarasota Area Hotel", "Hertz", "Delta"],
               1500, 4000, 0, 2)

        # Annual tax prep (CPA firm, billed in March)
        if m == 3:
            cen_txns.append(tx(centurion.id, clamp_day(y, m, 20), -D("3800.00"),
                               "Deloitte Tax Services", cat["tax_prep"]))

        # Annual property insurance Sarasota (June)
        if m == 6:
            add(tx(checking.id, clamp_day(y, m, 15), -D("9400.00"),
                   "Citizens Insurance — Sarasota", cat["home_insurance"]))
        ...
        if m == 7:
            add(tx(checking.id, clamp_day(y, m, 15), -D("5800.00"),
                   "Macon County NC Tax", cat["rental_property_tax"]))
        if m == 12:
            add(tx(checking.id, clamp_day(y, m, 20), -D("5800.00"),
                   "Macon County NC Tax", cat["rental_property_tax"]))

        # Sarasota home maintenance / landscaping (quarterly)
        if m in (1, 4, 7, 10):
            add(tx(checking.id, clamp_day(y, m, 14), -D("750.00"),
                   "Heritage Landscaping Sarasota", cat["lawn_garden"]))
        if rng.random() < 0.3:
            add(tx(checking.id, clamp_day(y, m, rng.randint(5, 25)),
                   -D(str(rng.randint(5, 30) * 100)),
                   "Sarasota Home Services", cat["home_maintenance"]))
        ...
        def sv(slug, merchants, min_t, max_t, min_n, max_n, **kw):
            sap_txns.extend(gen_variable(
                sapphire.id, y, m, cat[slug], merchants,
                D(str(min_t)), D(str(max_t)), min_n, max_n, rng, **kw))

        sv("restaurants",
           ["Owen's Fish Camp", "Brine Seafood & Raw Bar", "The Francis",
            "Farmer's Market Bistro", "Verbena Sarasota"],
           600, 1100, 3, 5)
        sv("groceries",
           ["Publix", "Whole Foods Sarasota", "Morton's Gourmet Market"],
           450, 650, 4, 7, avoid_sunday=True)
        sv("travel",
           ["Highlands NC Weekend", "Tampa Day Trip", "Clearwater Beach",
            "Visit Asheville NC"],
           300, 1200, 0, 2)
        sv("gas_fuel",
           ["Shell Sarasota", "Costco Gas", "BP"],
           120, 180, 3, 5)
        sv("fitness",
        ...
        cen_spend = abs(sum(t.amount for t in cen_txns if t.account_id == centurion.id
                            and t.amount < 0))
        if cen_spend > 0:
            d, c = transfer(checking.id, centurion.id, clamp_day(y, m, 28),
                            cen_spend, "AmEx Centurion Statement Payment",
                            cat["cc_payment"])
            add(d, c)

        sap_spend = abs(sum(t.amount for t in sap_txns if t.account_id == sapphire.id
                            and t.amount < 0))
        if sap_spend > 0:
            d, c = transfer(checking.id, sapphire.id, clamp_day(y, m, 28),
                            sap_spend, "Chase Sapphire Reserve Statement Payment",
                            cat["cc_payment"])
            add(d, c)
    ...
    session.add(make_debt(
        highlands_mort.id, D("375000.00"), D("342000.00"),
        D("0.0325"), D("1632.00"), 360, date(2019, 6, 4),
    ))
