...
async def seed(session: AsyncSession, rng: random.Random) -> dict:
    ...
    rev_trust = make_ownership_entity(
        hid, "revocable_trust", "Castellano Family Revocable Trust",
        counts_in_net_worth=True, in_taxable_estate=True, grantor_member_id=rosa.id,
    )
    ilit = make_ownership_entity(
        hid, "ilit", "Castellano Irrevocable Life Insurance Trust",
        counts_in_net_worth=False, in_taxable_estate=False, grantor_member_id=rosa.id,
    )
    crt = make_ownership_entity(
        hid, "crt_crut", "Castellano Charitable Remainder Unitrust",
        counts_in_net_worth=False, in_taxable_estate=False, grantor_member_id=rosa.id,
    )
    ...
    def acc(atype, name, inst, last4, *, in_nw=True, entity_id=None, revolving=False):
        a = make_account(
            hid, atype, name, inst, last4, owner_member_id=rosa.id,
            include_in_net_worth=in_nw, ownership_entity_id=entity_id, is_revolving=revolving,
        )
        session.add(a)
        return a
    ...
    diversified = acc("investment_brokerage", "Diversified Taxable Brokerage", "Fidelity PWM",
                      "5521", entity_id=rev_trust.id)
    ...
    whole_life_cv = acc("life_insurance_cash_value", "Whole Life Cash Value",
                        "Northwestern Mutual", "7720")
    art = acc("other_asset", "Art Collection (manually valued)", "—", None)
    pe_fund = acc("private_fund", "Meridian Private Equity Fund III NAV", "Meridian Capital", "8830")
    ...
    crt_brok = acc("investment_brokerage", "CRT Investment Account", "Fidelity Charitable",
                   "8841", in_nw=False, entity_id=crt.id)
    daf = acc("other_asset", "Castellano Donor-Advised Fund", "Fidelity Charitable", "8842",
              in_nw=False)
    await session.flush()

    # ── Real estate (titled in the revocable trust) ─────────────────────────────
    prop_residence = make_property(
        residence_re.id, "27 Heathcote Manor Rd, Scarsdale, NY 10583", "primary_residence",
        date(1998, 5, 1), D("950000.00"), ownership_entity_id=rev_trust.id,
    )
    prop_coop = make_property(
        coop_re.id, "180 Riverside Dr, Apt 8C, New York, NY 10024", "other",
        date(2005, 9, 1), D("740000.00"), ownership_entity_id=rev_trust.id,
    )
    session.add_all([prop_residence, prop_coop])
    await session.flush()
    for val_date, amt in [
        (date(2024, 1, 1), D("3650000.00")), (date(2024, 10, 1), D("3680000.00")),
        (date(2025, 6, 1), D("3740000.00")), (date(2026, 6, 1), D("3800000.00")),
    ]:
        session.add(make_valuation(prop_residence.id, val_date, amt))
    for val_date, amt in [
        (date(2024, 1, 1), D("1170000.00")), (date(2025, 1, 1), D("1185000.00")),
    ...
    snaps(diversified, [
        (date(2024, 1, 31), "3300000.00"), (dip[0], "3120000.00"),
        (date(2025, 6, 30), "3280000.00"), (date(2026, 5, 31), "3400000.00"),
    ])
    # Legacy concentrated position — single ticker, trimmed via scheduled sales.
    snaps(legacy_stock, [
        (date(2024, 1, 31), "2480000.00"), (date(2024, 10, 31), "2360000.00"),
        (date(2025, 6, 30), "2360000.00"), (date(2026, 5, 31), "2300000.00"),
    ])
    snaps(mm_cash, [(date(2024, 1, 31), "350000.00"), (date(2026, 5, 31), "400000.00")])
    # Rollover IRA decumulates via RMDs.
    snaps(rollover_ira, [
        (date(2024, 1, 31), "2950000.00"), (date(2024, 10, 31), "2880000.00"),
        (date(2025, 6, 30), "2840000.00"), (date(2026, 5, 31), "2800000.00"),
    ])
    # Inherited IRA on the SECURE 10-year clock (full depletion by 2033).
    snaps(inherited_ira, [
        (date(2024, 1, 31), "700000.00"), (date(2025, 6, 30), "660000.00"),
    ...
    snaps(pe_fund, [
        (date(2024, 1, 31), "980000.00"), (date(2025, 6, 30), "1120000.00"),
    ...
    session.add(make_investment_lot(legacy_stock.id, "INDP", D("18000"), D("83.33"),
                                    date(2022, 6, 15), "inherited_stepup"))  # basis ≈ 1,500,000
    session.add(make_investment_lot(diversified.id, "VTI", D("12000"), D("220.00"),
                                    date(2015, 3, 1), "purchase"))  # single synthetic lot ok

    # ── Capital commitment (private equity) ─────────────────────────────────────
    session.add(make_capital_commitment(
        hid, "Meridian Private Equity Fund III", D("2000000.00"), D("1300000.00"),
        pe_fund.id, 2021,
    ))
    ...
    session.add(make_insurance_policy(
        hid, "permanent_life", D("750000"), D("4200"), "quarterly",
        insured_member_id=rosa.id, cash_value_account_id=whole_life_cv.id,
    ...
    session.add(make_insurance_policy(
        hid, "permanent_life", D("3000000"), D("45000"), "annual",
        insured_member_id=rosa.id, owner_ownership_entity_id=ilit.id,
        metadata={"purpose": "estate_liquidity", "owned_by": "ilit"},
    ))
    session.add(make_insurance_policy(
        hid, "umbrella_liability", D("10000000"), D("2100"), "annual",
        metadata={"underlying": ["auto", "scarsdale_home", "manhattan_coop"]},
    ))
    session.add(make_insurance_policy(
        hid, "long_term_care", D("500000"), D("6200"), "annual", insured_member_id=rosa.id,
        metadata={"daily_benefit": 400, "inflation_rider": "3pct_compound"},
    ))
    session.add(make_insurance_policy(
        hid, "scheduled_specialty", D("800000"), D("4800"), "annual",
    ...
    running: dict[uuid.UUID, D] = {
        checking.id: D("0"), savings.id: D("0"), sbloc.id: D("0"),
    }
    ...
    for ms in all_months():
        ...
        # Income (to checking)
        add(tx(checking.id, clamp_day(y, m, 3), D("4200.00"), "SSA — Survivor Benefit",
               cat["social_security_income"]))
        add(tx(checking.id, clamp_day(y, m, 10), D("9800.00"), "Brokerage dividends + T-bill interest",
               cat["dividends"]))
        if m in (3, 6, 9, 12):  # CRT unitrust, quarterly (~$125K/yr)
            add(tx(checking.id, clamp_day(y, m, 15), D("31250.00"), "CRT unitrust distribution",
                   cat["crt_income"]))
            # Rollover-IRA RMD net of QCD: ~$60K/yr taxable cash, quarterly.
            add(tx(checking.id, clamp_day(y, m, 18), D("15000.00"), "RMD — Spousal Rollover IRA",
                   cat["rmd_distribution"]))
            # Inherited-IRA RMD (SECURE 10-year), quarterly.
            add(tx(checking.id, clamp_day(y, m, 20), D("16000.00"), "RMD — Inherited IRA",
        ...
        add(tx(checking.id, clamp_day(y, m, 2), -part_b, "Medicare Part B (IRMAA top tier)",
               cat["medicare_part_b"]))
        add(tx(checking.id, clamp_day(y, m, 2), -part_d, "Medicare Part D (IRMAA top tier)",
               cat["medicare_part_d"]))
        add(tx(checking.id, clamp_day(y, m, 3), -D("310.00"), "UnitedHealthcare Medigap Plan G",
               cat["medigap_supplement"]))

        # Co-op maintenance (monthly) + Westchester property tax (semiannual ~$60K/yr)
        add(tx(checking.id, clamp_day(y, m, 1), -D("2650.00"), "Manhattan Co-op Maintenance",
               cat["hoa_fees"]))
        if m in (1, 7):
            add(tx(checking.id, clamp_day(y, m, 10), -D("30000.00"), "Westchester County Property Tax",
                   cat["home_property_tax"]))

        # SBLOC interest (monthly) once drawn
        if (y, m) >= (2024, 5):
            add(tx(sbloc.id, clamp_day(y, m, 28), -D("2300.00"), "SBLOC interest",
                   cat["sbloc_interest"]))

    # ── Discrete events ───────────────────────────────────────────────────────
    # PE capital calls against the outstanding commitment, and one distribution.
    add(*transfer(mm_cash.id, pe_fund.id, date(2024, 3, 14), D("150000.00"),
                  "Meridian PE — capital call", cat["capital_call"]))
    add(*transfer(mm_cash.id, pe_fund.id, date(2024, 11, 12), D("180000.00"),
                  "Meridian PE — capital call", cat["capital_call"]))
    add(tx(checking.id, date(2025, 9, 18), D("220000.00"), "Meridian PE — distribution",
    ...
    for gift_year in (2024, 2025, 2026):
        if date(gift_year, 1, 1) <= DATE_END:
            add(tx(checking.id, clamp_day(gift_year, 1, 20), -D("95000.00"),
                   "Annual-exclusion gifts (children & grandchildren)", cat["annual_exclusion_gift"]))

    # Annual gift to ILIT funding the ~$45K premium (Crummey-style).
    for gift_year in (2024, 2025, 2026):
        if date(gift_year, 1, 1) <= DATE_END:
            add(tx(checking.id, clamp_day(gift_year, 2, 15), -D("45000.00"),
                   "Gift to ILIT (premium funding)", cat["gift_to_ilit"]))

    # Systematic diversification trims of the legacy concentrated position.
    for ms in all_months():
        y, m = ms.year, ms.month
        if m in (3, 9) and ms <= DATE_END:
            add(tx(legacy_stock.id, clamp_day(y, m, 12), -D("60000.00"),
                   "Scheduled diversification sale (legacy stock)", cat["equity_sale"]))
    ...
    for qcd_year in (2025, 2026):
        if date(qcd_year, 11, 1) <= DATE_END:
            add(tx(rollover_ira.id, clamp_day(qcd_year, 11, 15), -D("50000.00"),
                   "QCD — direct to public charity", cat["qcd_note"]))

    # Other premiums + a modest art acquisition.
    for prem_year in (2024, 2025, 2026):
        add(tx(checking.id, date(prem_year, 1, 12), -D("2100.00"), "Chubb Umbrella Policy",
               cat["umbrella_premium"]))
        add(tx(checking.id, date(prem_year, 1, 14), -D("4800.00"), "AXA Art Schedule",
               cat["specialty_insurance_premium"]))
        if prem_year < 2026:
            add(tx(checking.id, date(prem_year, 9, 10), -D("6200.00"), "Genworth LTC",
                   cat["ltc_insurance_premium"]))
        for q in (3, 6, 9, 12):
            if date(prem_year, q, 1) <= DATE_END:
                add(tx(checking.id, clamp_day(prem_year, q, 14), -D("4200.00"),
                       "Northwestern Mutual Whole Life", cat["permanent_life_premium"]))
    add(tx(checking.id, date(2025, 5, 8), -D("85000.00"), "Sotheby's — art acquisition",
           cat["hobbies"]))
    ...
    for acc_id, target in targets.items():
        session.add(opening_balance_tx(acc_id, target - running[acc_id], cat.get("between_accounts")))

    session.add_all(all_txns)

    # ── FIRE scenario — single-life decumulation ────────────────────────────────
    fire = make_fire_scenario(
        hid, rosa.id, "Single-Life Decumulation (estate-exposed)",
        D("420000.00"), D("0.0500"), D("0.0300"), 95,
        [
            {"id": str(uuid.uuid4()), "label": "SSA survivor benefit", "type": "social_security",
             "amount_annual": 50400.00, "start_year": 2024, "end_year": None,
             "growth_rate_annual": 0.025, "is_pre_retirement": False},
            {"id": str(uuid.uuid4()), "label": "CRT unitrust income", "type": "other",
             "amount_annual": 125000.00, "start_year": 2024, "end_year": None,
             "growth_rate_annual": 0.0, "is_pre_retirement": False},
            {"id": str(uuid.uuid4()), "label": "RMDs + dividends + T-bill interest",
             "type": "investment", "amount_annual": 178000.00, "start_year": 2024,
             "end_year": None, "growth_rate_annual": 0.0, "is_pre_retirement": False},
            {"id": str(uuid.uuid4()), "label": "Inherited-IRA RMD (declining, depletes 2033)",
             "type": "investment", "amount_annual": 64000.00, "start_year": 2024,
             "end_year": 2033, "growth_rate_annual": -0.08, "is_pre_retirement": False},
            {"id": str(uuid.uuid4()), "label": "PE distributions (irregular)", "type": "other",
             "amount_annual": 70000.00, "start_year": 2024, "end_year": 2028,
             "growth_rate_annual": 0.0, "is_pre_retirement": False},
        ],
